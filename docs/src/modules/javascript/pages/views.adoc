...
* A `table` attributed that can be any name. Use this name in the query `SELECT` statement.
...
<3> The `(akkaserverless.method).view.update` annotation defines that this method is used for updating the View. You must define the `table` attribute for the table to be used in the query. Use any name, which you will reference in the query `SELECT` statement.
...
* Specifies a `table` attribute used by Akka Serverless to store the View. Use any name, which you will reference in the Query `SELECT` statement.
