...
* A `table` property that can be any name. Use this name in the query `SELECT` statement.
...
<3> The `(akkaserverless.method).view.update` annotation defines that this method is used for updating the View. You must define the `table` property for the table to be used in the query. You can use any name, which will be referenced in the `SELECT` statement.
